...
import { cn } from "cn"
import type { EvalCategory, EvalDimension } from "@/types/console"
...
export function EvaluationsView({
  ...
  const filtered = filter === "all" ? rows : rows.filter((s) => s.category === filter)
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-6 lg:px-6">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          {rows.length} scenarios across normal, edge, incorrect-information, adversarial, tool-failure and
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[auto_1fr]">
        <div className="glass-panel flex flex-col items-center justify-center p-6 lg:w-48">
          <p className="tabular text-4xl font-semibold text-primary">{overallScore != null ? `${overallScore}%` : "—"}</p>
        ...
        <div className="glass-panel flex flex-col justify-center gap-2.5 p-4">
          {dimensions.length === 0 && <p className="text-xs text-muted-foreground">No evaluation runs recorded yet.</p>}
          {dimensions.map((d) => (
            <div key={d.label}>
            ...
            {filtered.map((s) => (
              <tr key={s.id} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-2.5">
                  {s.result ? (
                    <Badge variant={s.result === "failed" ? "destructive" : "approve"}>
