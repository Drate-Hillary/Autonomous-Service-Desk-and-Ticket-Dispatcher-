...
import { dashboardStats, evalDimensions, traceRuns } from "@/lib/mock-console"
import {
  Pulse02Icon,
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 lg:px-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <StatCard label="Tasks today" value={String(dashboardStats.tasksToday)} icon={CheckmarkCircle02Icon} />
        <StatCard label="Success rate" value={`${dashboardStats.successRate}%`} icon={Target01Icon} />
        <StatCard label="Avg latency" value={`${(dashboardStats.avgLatencyMs / 1000).toFixed(1)}s`} icon={Time01Icon} />
        <StatCard label="Eval scenarios" value={String(dashboardStats.evalScenarios)} icon={Chart01Icon} />
        <StatCard label="RAG documents" value={String(dashboardStats.ragDocuments)} icon={Knowledge01Icon} />
        <StatCard label="Active tools" value={String(dashboardStats.activeTools)} icon={Wrench01Icon} />
        <StatCard label="Memory records" value={String(dashboardStats.memoryRecords)} icon={AiBrain01Icon} />
        <StatCard label="Failed runs (7d)" value={String(dashboardStats.failedRuns)} icon={Alert02Icon} tone="danger" />
        <StatCard label="Pending approvals" value={String(dashboardStats.pendingApprovals)} icon={SecurityCheckIcon} tone="warn" />
      </div>
            ...
            {traceRuns.map((run) => (
              <li key={run.id} className="flex items-center justify-between gap-3 py-2.5 first:pt-0 last:pb-0">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm text-muted-foreground">{run.date}</p>
                </div>
                <span className="flex shrink-0 items-center gap-1.5 text-sm text-muted-foreground">
                  <span className={`size-1.5 rounded-full ${statusDot[run.status]}`} />
